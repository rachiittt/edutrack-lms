 ...
 * 
 * SOLID:
 * - SRP: Only handles course business logic (no DB queries here).
 * - DIP: Depends on ICourseRepository abstraction.
 * - OCP: New course behaviors can be added via subclassing.
 * 
 * PATTERNS:
 * - Observer: Publishes events on creation/deletion.
...
export class CourseService implements ICourseService {
  ...
  async update(id: string, teacherId: string, data: Partial<ICourse>): Promise<ICourse> {
    ...
    // Authorization check (business logic, not DB concern)
